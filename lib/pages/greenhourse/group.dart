...
class _GroupPageState extends State<GroupPage> {
  ...
  void _fetchData() async {
    ...
    final response = await ApiService.fetchGroupAll();
    if (!mounted) return;
            ...
            builder: (context) {
              return StatefulBuilder(
                builder: (context, setStateDialog) {
                  return AlertDialog(
                    title: SizedBox(height: 40, child: Text("สร้าง Group ใหม่")),
                    content: Container(
                      width: maxwidth,
                      height: minheight,
                      child: Column(
                        children: [
                          SizedBox(width: maxwidth, child: Text("ชื่อ Group")),
                          Expanded(
                            child: TextField(
                              onChanged: (value) {
                                setState(() {
                          ...
                          Expanded(
                            child: DropdownButtonFormField(
                              ...
                              isExpanded: true,
                              hint: Text("เลือก branch ของ group"),
                          ...
                          if (newname['newname'] != '') {
                            ...
                            if (response['status'] == 'success') {
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text("Group ใหม่ ${newname['newname']} ถูกสร้างแล้ว! ")),
                              );
                            } else {
                            ...
                            ScaffoldMessenger.of(
                              context,
                            ).showSnackBar(SnackBar(content: Text("Group ใหม่ จำเป็นต้องมีชื่อ! ")));
                          }
                          newname['newname'] = '';
                          Navigator.of(context).pop();
                        },
                    ...
                    child: Row(
                      children: [
                        SizedBox(
                          width: maxwidth * 0.1,
                          child: Center(
                            child: Text(item['group_id'], style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                          ),
                        ),
                        Expanded(
                                            ...
                                            children: [
                                              ...
                                              Row(
                                                children: [
                                                  ...
                                                  Expanded(
                                                    child: Text(item['group_name'], style: TextStyle(fontSize: 16)),
                                                  ),
                                                ],
                                              ),
                                              SizedBox(height: 20),
                                            ...
                                            onPressed: () async {
                                              ...
                                              if (response['status'] == 'success') {
                                                ScaffoldMessenger.of(context).showSnackBar(
                                                  SnackBar(
                                                    content: Text(
                                                      "ทำการเปลี่ยนชื่อเป็น ${groups[index]['group_name']} เรียบร้อยแล้ว.",
                                                    ),
                                                  ),
                                                );
                                                Navigator.of(context).pop();
                                      ...
                                      return AlertDialog(
                                        ...
                                        content: SizedBox(
                                          ...
                                          child: Center(
                                            child: Column(
                                              children: [
                                                Text('คุณต้องการลบ ${item['group_name']} ใช่หรือไม่?'),
                                                SizedBox(height: 20),
                                                Text(
